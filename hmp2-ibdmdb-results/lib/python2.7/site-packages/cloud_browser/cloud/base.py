...
class CloudObject(object):
    ...
    def smart_content_type(self):
        ...
        # KRB add more recent mime.types than what's avail in CentOS 5.5
        if "/local/ibdmdb/etc/mime.types" not in mimetypes.knownfiles:
            mimetypes.knownfiles.append("/local/ibdmdb/etc/mime.types")
            #import sys; print >> sys.stderr, mimetypes.knownfiles
    ...
    def smart_content_encoding(self):
        ...
        if not encoding:
            ...
            # KRB
            # just test the last extension - not all of them
            extension = self.basename.rpartition('.')[2]
            _, encoding = mimetypes.guess_type('.'.join(extension))
